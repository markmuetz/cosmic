...
def gen_raster_cube(inputs, outputs, hb_name, hbs, method):
    cube = iris.load_cube(str(inputs[0]))
    if method == 'global':
        # Build global raster, then extract Asia.
        raster_cube = util.build_raster_cube_from_cube(cube, hbs[hb_name], hb_name)
        ...
        raster_cube_asia = util.build_raster_cube_from_cube(cube_asia, hbs[hb_name], hb_name)
    iris.save(raster_cube_asia, str(outputs[0]))


def plot_raster_cube(inputs, outputs, hb_name, hbs, method):
    raster_cube_asia = iris.load_cube(str(inputs[0]))
    ...
    ax = plt.gca()
    hbs[hb_name].geometry.boundary.plot(ax=ax, color=None, edgecolor='r')
    plt.xlim((50, 160))
    plt.ylim((0, 60))
...
def gen_raster_stats(inputs_dict, outputs, hbs):
    output_text = []
    for model in MODELS:
...
if __name__ == '__main__':
    ...
    for model in MODELS:
        for hb_name in ['small', 'med', 'large']:
            input_filename = FILENAMES[model]
    ...
    for model in MODELS:
        for hb_name in ['small', 'med', 'large']:
            diff_methods = task_ctrl.index[(task_ctrl.index.model == model) & (task_ctrl.index.hb_name == hb_name)]
